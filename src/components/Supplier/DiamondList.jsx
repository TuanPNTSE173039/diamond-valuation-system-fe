...
const DiamondList = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useDiamondsOfSupplier(page, rowsPerPage, supplierId);
  ...
  const handleDeleteDiamond = async (id) => {
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: ["diamonds", { page, rowsPerPage, supplierId }],
      });
      await refetch();
    } catch (error) {
      toast.success("Delete diamond market successfully");
  ...
  return (
    <>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setOpenDeleteConfirm(false)} variant="text">
            Cancel
          </Button>
          <Button
            onClick={() => handleDeleteDiamond(selectedDeleteId)}
            variant="contained"
            color="secondary"
          >
            Delete
